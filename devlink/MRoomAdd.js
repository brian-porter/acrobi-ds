"use client";
import React from "react";
import * as _Builtin from "./_Builtin";
import { SecHead } from "./SecHead";
import { SelectlistForm } from "./SelectlistForm";
import { TextfieldForm } from "./TextfieldForm";
import { FieldLabel } from "./FieldLabel";
import { Cell } from "./Cell";
import { FieldHelper } from "./FieldHelper";
import { Spacer } from "./Spacer";
import { TextareaForm } from "./TextareaForm";
import { ChipsSs } from "./ChipsSs";
import { Button } from "./Button";
import { InputWBtns } from "./InputWBtns";
import * as _utils from "./utils";
import _styles from "./MRoomAdd.module.css";

export function MRoomAdd({
  as: _Component = _Builtin.Block,
  cancelClick = {},
  typePHoldSrc = "Forum",
  typePHoldClr = "n500",
  typeSelectDrpHide = false,
  typeSelectMap,
  sectionPHoldSrc = "Forum",
  sectionPHoldClr = "n500",
  sectionSelectDrpHide = false,
  sectionSelectMap,
  nameChange,
  tagChange,
  badgeMap,
  badgeExample = true,
  descChange,
  accessMap,
  accessExampleMap = true,
  accessAddClick = {},
  erase = false,
  eraseClick = {},
  doClick = {},
}) {
  return (
    <_Component
      className={_utils.cx(_styles, "g-add-room")}
      tag="section"
      mini=""
      id="Room"
    >
      <_Builtin.Block className={_utils.cx(_styles, "a-header")} tag="div">
        <SecHead
          act1Click={cancelClick}
          titleSrc="Room Properties"
          sz="xl"
          act1={true}
          titleSz="h4"
          subtxt={true}
          subtxtSrc="Giving a clear name and purpose for the room will help members understand the context of the room, for better engagement."
        />
      </_Builtin.Block>
      <_Builtin.Block className={_utils.cx(_styles, "a-body2")} tag="div">
        <_Builtin.FormWrapper>
          <_Builtin.FormForm
            name="email-form"
            data-name="Email Form"
            method="get"
            id="email-form"
          >
            <SelectlistForm
              fieldPHoldSrc={typePHoldSrc}
              fieldPHoldClr={typePHoldClr}
              fieldSelectMap={typeSelectMap}
              fieldSelectDrpHide={typeSelectDrpHide}
              lblTopLblSrc="Room Type"
              fldHelp={false}
              fieldFldId="type"
            />
            <SelectlistForm
              fieldPHoldSrc={sectionPHoldSrc}
              fieldPHoldClr={sectionPHoldClr}
              fieldSelectMap={sectionSelectMap}
              fieldSelectDrpHide={sectionSelectDrpHide}
              lblTopLblSrc="Section"
              fldHelp={false}
              fieldFldId="section"
            />
            <TextfieldForm
              fieldOnChange={nameChange}
              fldHelpHelpLSrc=""
              lblTopOpt={true}
              lblTopOptSrc="required"
              fldHelpHelpRSrc="0/60"
              fldHelpHelpR={true}
              lblTopLblSrc="Name"
              fieldFldPlaceholderSrc="group name"
              fldHelp={true}
              fieldTabOrder="0"
            />
            <TextfieldForm
              fieldOnChange={tagChange}
              fldHelpHelpLSrc=""
              lblTopOpt={true}
              lblTopOptSrc="required"
              fldHelpHelpRSrc="0/60"
              fldHelpHelpR={true}
              lblTopLblSrc="Tag"
              fieldFldPlaceholderSrc="#room name"
              fldHelp={true}
              fieldTabOrder="0"
            />
            <_Builtin.Block
              className={_utils.cx(_styles, "form_main")}
              tag="div"
            >
              <FieldLabel
                fldLblTop={true}
                lblSrc="Room Symbol"
                optSrc="required"
                lblFor=""
              />
              <_Builtin.Block
                className={_utils.cx(_styles, "badgebar")}
                tag="div"
              >
                <_Builtin.Block
                  className={_utils.cx(_styles, "badgebar-ss")}
                  tag="div"
                >
                  <_Builtin.Grid
                    className={_utils.cx(_styles, "badge-grid")}
                    tag="div"
                  >
                    {badgeMap ?? (
                      <Cell
                        cell={badgeExample}
                        cellSz="m"
                        vizAdpt={true}
                        vizImg={false}
                        captionPriceBdg={false}
                        captionCapStk={true}
                        capStkRow1Src="ItemName"
                        adptAdptSz="m"
                        capStkRowsAlign="c"
                        adptAdptBgClr="n500"
                        caption={false}
                      />
                    )}
                  </_Builtin.Grid>
                </_Builtin.Block>
              </_Builtin.Block>
              <FieldHelper
                helpLSrc=""
                helpRSrc="0/20"
                helpR={true}
                helpL={true}
                fldHelp={false}
              />
            </_Builtin.Block>
            <Spacer size="16" />
            <TextareaForm
              fieldOnChange={descChange}
              lblTopLblSrc="Description"
              fldHelpHelpLSrc="give the purpose of this role"
              fldHelpHelpR={true}
              fldHelpHelpRSrc="0/100"
              fieldTabOrder="0"
            />
            <SecHead titleSrc="Access" act1={false} titleSz="r2" />
            <_Builtin.Block
              className={_utils.cx(_styles, "form-columns")}
              tag="div"
            >
              <ChipsSs cellMap={accessMap} exampleCells={accessExampleMap} />
              <Button
                btnClick={accessAddClick}
                btnTxtSrc="Add"
                btnSz="m"
                btnIcnSrc="Add"
                btnStyl="nl"
              />
            </_Builtin.Block>
          </_Builtin.FormForm>
          <_Builtin.FormSuccessMessage>
            <_Builtin.Block tag="div">
              {"Thank you! Your submission has been received!"}
            </_Builtin.Block>
          </_Builtin.FormSuccessMessage>
          <_Builtin.FormErrorMessage>
            <_Builtin.Block tag="div">
              {"Oops! Something went wrong while submitting the form."}
            </_Builtin.Block>
          </_Builtin.FormErrorMessage>
        </_Builtin.FormWrapper>
      </_Builtin.Block>
      <_Builtin.Block
        className={_utils.cx(_styles, "a-footer")}
        tag="div"
        data-top-shadow="y"
      >
        <InputWBtns
          lLBtn={erase}
          lLBtnClick={eraseClick}
          tTBtnClick={doClick}
          tTBtn={true}
          pPFld={false}
          lLBtnIcnSrc="delete"
          tTBtnTxt={true}
          tTBtnIcn={false}
          tTBtnTxtSrc="Done"
          tTBtnIcnSrc="default"
          tTBtnStyle="pf"
        />
        <Spacer size="16" />
      </_Builtin.Block>
    </_Component>
  );
}
