"use client";
import React from "react";
import * as _Builtin from "./_Builtin";
import { SecHead } from "./SecHead";
import { ItemHead } from "./ItemHead";
import { Spacer } from "./Spacer";
import { TextfieldForm } from "./TextfieldForm";
import { InputWBtns } from "./InputWBtns";
import * as _utils from "./utils";
import _styles from "./MPostVidAdd.module.css";

export function MPostVidAdd({
  as: _Component = _Builtin.Block,
  cancelClick = {},
  itmHead = true,
  itmImgSrc = "https://cdn.prod.website-files.com/6466482759c1cbf5069c78d5/6466482759c1cbf5069c790c_image.svg",
  itmImgAlt = "__wf_reserved_inherit",
  itmName = "ItemName description goes here andline wraps",
  vidLinkClick = {},
  vidLinkChange,
  doClick = {},
}) {
  return (
    <_Component className={_utils.cx(_styles, "g-postvid-add")} tag="section">
      <_Builtin.Block className={_utils.cx(_styles, "a-header")} tag="div">
        <SecHead
          act1Click={cancelClick}
          titleSrc="Add Video"
          titleSz="h4"
          sz="m"
        />
        <ItemHead
          itmHead={itmHead}
          itmImgSrc={itmImgSrc}
          name={itmName}
          itmImgAlt={itmImgAlt}
        />
      </_Builtin.Block>
      <_Builtin.Block className={_utils.cx(_styles, "a-body")} tag="div">
        <Spacer />
        <_Builtin.FormWrapper>
          <_Builtin.FormForm
            name="wf-form-Post-2"
            data-name="Post"
            method="get"
            id="wf-form-Post-2"
          >
            <TextfieldForm
              fieldOnChange={vidLinkChange}
              fieldFldClick={vidLinkClick}
              lblTopLblSrc="Video Link"
              fldHelp={true}
              fieldTabOrder="0"
              fld={true}
              fldHelpHelpLSrc="Copy and paste a YouTube url"
            />
            <Spacer szDep="64" />
          </_Builtin.FormForm>
          <_Builtin.FormSuccessMessage>
            <_Builtin.Block tag="div">
              {"Thank you! Your submission has been received!"}
            </_Builtin.Block>
          </_Builtin.FormSuccessMessage>
          <_Builtin.FormErrorMessage>
            <_Builtin.Block tag="div">
              {"Oops! Something went wrong while submitting the form."}
            </_Builtin.Block>
          </_Builtin.FormErrorMessage>
        </_Builtin.FormWrapper>
      </_Builtin.Block>
      <_Builtin.Block
        className={_utils.cx(_styles, "a-footer")}
        tag="div"
        bs="s"
      >
        <InputWBtns
          tTBtnClick={doClick}
          lLBtn={false}
          tTBtnTxtSrc="Done"
          tTBtnTxt={true}
          tTBtnStyle="pf"
          tTBtnSz="l"
          tTBtnIcn={false}
          lLBtnIcnSrc="emj"
          pPToolbar={false}
          lLBtnClick={{}}
          pPFld={false}
        />
        <Spacer />
      </_Builtin.Block>
    </_Component>
  );
}
